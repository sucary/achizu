 ...
 * that don't yet exist in locations.
 */
async function seedPriorityCities() {
    await verifyDatabaseConnection();

    try {
        // Get priority locations that aren't in locations yet
        ...
        if (result.rows.length === 0) {
            console.log('All priority cities already exist in locations.');
            return;
        }
        ...
        for (const row of result.rows) {
            ...
            try {
                ...
                // Save to locations (with water polygon cutting)
